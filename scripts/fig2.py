...
def get_v_snnap(dt,snnap_datapath):
    ...
    """
    data = pd.read_csv(os.path.join(snnap_datapath,f"data_{dt}.out"), sep="\t").dropna(axis=1).dropna(axis=0)
...
if __name__ == "__main__":
    ...
    ax1.legend(loc="upper left",frameon=False,fontsize=22)
    ...
    ax2.legend(handles=plt.gca().get_legend_handles_labels()[0] + [custom_line],loc="upper left",frameon=False,fontsize=22)

    # Label axes
    ax2.set_ylabel("RMSE", fontsize=30)
    ax2.set_xlabel("dt (ms)", fontsize=30)
